  ...
  return (
    <div className="h-full overflow-y-auto bg-[#F8FAFC] dark:bg-[#0F172A] transition-colors duration-200">
      <div className="max-w-3xl mx-auto px-8 py-8">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-display text-[#0F172A] dark:text-[#F8FAFC]">Quiz generator</h1>
        ...
        <div className="p-6 mb-6 rounded-2xl border bg-white dark:bg-[#1E293B] border-[#E2E8F0] dark:border-[#334155] shadow-sm">
          <h2 className="text-title text-[#0F172A] dark:text-[#F8FAFC] mb-4">Configure quiz</h2>
          ...
          {/* Timed mode */}
          <div className="flex items-center justify-between p-4 rounded-xl mb-4 bg-[#F8FAFC] dark:bg-[#0F172A] border border-[#E2E8F0] dark:border-[#334155]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-[#0F172A] dark:text-[#F8FAFC]">Timed exam mode</p>
        ...
        {timerActive && timeLeft > 0 && (
          <div className="p-4 mb-6 flex items-center gap-4 rounded-2xl border bg-white dark:bg-[#1E293B] border-[#E2E8F0] dark:border-[#334155] shadow-sm">
            ...
            <div className="flex-1">
              <div className="flex justify-between text-xs mb-1.5">
                <span className="font-medium text-[#0F172A] dark:text-[#F8FAFC]">Time remaining</span>
        ...
        {/* Score banner */}
        {score && (
          <div className="p-5 mb-6 rounded-2xl border shadow-sm"
            style={{
              background: score.correct / score.total >= 0.7 ? 'var(--score-good-bg)' : 'var(--score-bad-bg)',
            }}
          >
            <style>{`:root{--score-good-bg:#F0FDF4;--score-bad-bg:#FEF2F2}.dark{--score-good-bg:rgba(16,185,129,0.1);--score-bad-bg:rgba(239,68,68,0.1)}`}</style>
        ...
        {questions.length > 0 && (
          <div className="flex flex-col gap-4">
            {questions.map((q, qi) => (
              <div key={qi} className="p-5 rounded-2xl border bg-white dark:bg-[#1E293B] border-[#E2E8F0] dark:border-[#334155] shadow-sm">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    <p className="text-sm font-medium text-[#0F172A] dark:text-[#F8FAFC] leading-relaxed">{q.question}</p>
                    <div className="flex gap-1.5 mt-2">
                      <span className="badge badge-indigo">{q.topic || 'Engineering'}</span>
                      <span className="badge bg-[#F8FAFC] dark:bg-[#0F172A] text-[#64748B] dark:text-[#94A3B8]">{q.difficulty}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="ml-10">
                    ...
                    {evaluations[qi] && (
                      <div className="mt-3 px-4 py-3 rounded-xl text-xs leading-relaxed"
                        style={{
                          background: evaluations[qi].is_correct ? 'rgba(16,185,129,0.08)' : 'rgba(239,68,68,0.08)',
                          borderLeft: `3px solid ${evaluations[qi].is_correct ? '#10B981' : '#EF4444'}`,
                        }}>
                        <p className="font-semibold mb-1 text-[#0F172A] dark:text-[#F8FAFC]">Score: {evaluations[qi].score}/100 · {evaluations[qi].is_correct ? 'Correct' : 'Needs review'}</p>
