...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [quizHistory, fcStats, sessions] = await Promise.all([
          getQuizHistory(),
          getFlashcardStats(),
          getSessions(),
        ])
        ...
        setStats({
          ...
          avgScore,
          flashcards: fcStats.total,
          dueToday: fcStats.due_today,
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            {/* Quick actions */}
            <div className="bg-[#18181f] border border-[#222230] rounded-xl p-4">
              ...
              <div className="flex flex-col gap-2">
                {quickActions.map((action) => (
                  <button
                    ...
                    className="flex items-center gap-3 p-3 rounded-lg bg-[#222230] hover:bg-[#2a2a3a] transition-colors text-left group"
                  >
                    <span className="text-lg">{action.icon}</span>
                    <div>
                      <div className="text-sm font-medium group-hover:text-[#7c6af7] transition-colors">
                        {action.label}
                      </div>
                      <div className="text-xs text-gray-500">{action.desc}</div>
                    </div>
                    <span className="ml-auto text-gray-600 group-hover:text-[#7c6af7] transition-colors">→</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="bg-[#18181f] border border-[#222230] rounded-xl p-4">
              <h2 className="text-sm font-semibold mb-3">
                Weak topics
                <span className="ml-2 text-xs text-gray-500 font-normal">below 60% accuracy</span>
              </h2>
              ...
              {weakTopics.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-center">
                  ...
                  <p className="text-xs text-gray-600 mt-1">
                    Take a quiz to see your topic breakdown
                  </p>
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                  ...
                  <button
                    ...
                    className="mt-2 text-xs text-[#7c6af7] hover:underline text-left"
                  >
